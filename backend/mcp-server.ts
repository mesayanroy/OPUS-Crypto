...
// MCP Server for OPUS AI Trading Platform
// Handles agent trigger evaluation and execution on local machine

interface AgentTrigger {
  type: "price_above" | "price_below" | "price_change" | "volume_above" | "time_interval" | "manual";
  targetValue: number;
  tolerance?: number;
}

interface TradeAction {
  type: "swap" | "limit_order";
  tokenIn: string;
  tokenOut: string;
  amount: number;
  slippage: number;
...
class MCPServer {
  ...
  private setupRoutes() {
    ...
    this.app.post("/api/batch-execute", async (req, res) => {
      ...
      const results = await Promise.all(
        rules.map((rule: ExecutionRule) => this.executeRule(rule))
      );
  ...
  private validateRuleExecution(rule: ExecutionRule, walletAddress: string): boolean {
  ...
  private async executeRule(rule: ExecutionRule): Promise<any> {
  ...
  private async handleAgentQuery(query: string): Promise<string> {
...
Provide concise, actionable advice.`,
  ...
  private async analyzeWithAI(query: string, agentMetrics?: any): Promise<string> {
...
export { ExecutionRule, AgentTrigger, TradeAction };
