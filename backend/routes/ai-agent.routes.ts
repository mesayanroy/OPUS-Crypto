...
import AIAgent from "../models/AIAgent.model"
...
router.get("/", authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const agents = await AIAgent.find({ userId }).sort({ updatedAt: -1 })
...
router.delete("/:id", authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const agent = await AIAgent.findOneAndDelete({ _id: id, userId })
...
router.post("/:id/toggle", authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const agent = await AIAgent.findOne({ _id: id, userId })
    if (!agent) {
      return res.status(404).json({
        success: false,
        message: "Agent not found",
